    ...
    while ((c = getopt_long(argc, argv, "", opts, &index)) != -1)
    {
        ...
        case 'e':
            if (!optarg)
            {
                ...
                {
                    service_t *serviceOfNet =(service_t *)malloc(sizeof(service_t));
                    int i = 0;
                     _get_all_devices();
                    ...
                    for (pcap_if_t *dev = g_device; dev; dev = dev->next)
                    {
                        i++;
                        serviceOfNet->id = i-1;
                        strncpy(serviceOfNet->description, &dev->description, sizeof(dev->description));
                        strncpy(serviceOfNet->name, &dev->name, sizeof(dev->name));
                        serviceOfNet = serviceOfNet->next;
                        serviceOfNet = (serviceOfNet *)malloc(sizeof(service_t));
                    }
                    while(serviceOfNet)
                    {
                        printf("%d\t%s\n",serviceOfNet->id, serviceOfNet->description);
                        printf("%s\n", serviceOfNet->name);
                        serviceOfNet->next;
                        getchar();
                    }
